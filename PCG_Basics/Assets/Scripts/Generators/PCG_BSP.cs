...
namespace PCG
{
    ...
    public class BSP_Tree
    {
        ...
        {
            if (Utility.RNDG.Next(0, 2) == 0)//Cut along X axis
                CutX_B = Utility.RNDG.Next(PCG_BSP.MinimumArea, partition.GetLength(0) - PCG_BSP.MinimumArea + 1);
            else//Cut along Y axis
                CutY_B = Utility.RNDG.Next(PCG_BSP.MinimumArea, partition.GetLength(1) - PCG_BSP.MinimumArea + 1);
        }
        ...
        {
            //Starting Point
            int startX = Utility.RNDG.Next(0, _partition.GetLength(0) - PCG_BSP.MinimumArea + 1);
            int startY = Utility.RNDG.Next(0, _partition.GetLength(1) - PCG_BSP.MinimumArea + 1);

            //Ending point: any random position from the starting point, but still greater than the minium area
            int endX = Utility.RNDG.Next(PCG_BSP.MinimumArea + startX, _partition.GetLength(0) + 1);
            int endY = Utility.RNDG.Next(PCG_BSP.MinimumArea + startY, _partition.GetLength(1) + 1);

            _room = new Room(new List<Cell>(), Room.RoomType.Room, "Room");
        ...
        private void Set_CombinedRoom()
        {
            ...
            Room combinedRoom = new Room(new List<Cell>(), Room.RoomType.Room, "Combined Room");//This room is bubbled up through the tree
        ...
        private void Create_ConnectingCorridor(Room combinedRoom)
        {
            ...
            Room corridorRoom = new Room(new List<Cell>(), Room.RoomType.Corridor, "Corridor");//This corridor is added as a room in case we want to acess the corridor seperatly
            PCG_BSP.Rooms.Add(corridorRoom);

            //Randomly choose a point from each room
            Cell startCell = LeftLeaf._room.RoomCells[Utility.RNDG.Next(0, LeftLeaf._room.RoomCells.Count)];
            Cell endCell = RightLeaf._room.RoomCells[Utility.RNDG.Next(0, RightLeaf._room.RoomCells.Count)];
